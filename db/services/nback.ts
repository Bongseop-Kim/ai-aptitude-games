...
export const saveNbackGameData = async ({ summaryList, trialsList, type }: saveNbackGameDataParams) => {
    await db.transaction(async (tx) => {
        ...
        for (const summary of summaryList) {
            // 2. 스테이지 저장
            const [stage] = await tx.insert(stages).values({
            ...
            for (const [n, data] of Object.entries(summary.perOffset)) {
                const offsetData = data as { total: number; correct: number; avgRtMs: number | null };
