...
- 파일명: `kebab-case.ts`
- 컴포넌트명: `PascalCase`
- 훅 파일명: `use-*.ts` 형태의 `kebab-case` (e.g. `use-nback-game.ts`)
- 훅 export 심볼명: `use` 접두사의 `camelCase` 식별자 (e.g. `useNBackGame`)
- 슬라이스 디렉토리명: `kebab-case` (e.g. `nback-game`, `nback-history`)

예시 매핑:

```typescript
// features/nback-game/model/use-nback-game.ts
...
`app/` 파일은 가능한 한 얇은 라우트 셸로 유지합니다. 기본 역할은 해당 Widget을 import해 렌더링하는 것이지만,
라우트 경계에서 필요한 최소한의 로직은 허용됩니다. 예를 들어 `app/games/nback/detail/[id]/index.tsx`,
`app/games/nback/summary/[id]/index.tsx` 같은 페이지 레이어에서는 경로 파라미터 검증, 잘못된 ID에 대한 리디렉션,
접근 불가 상태 확인 같은 라우트 경계 로직을 둘 수 있습니다. 반면 데이터 처리, 계산, 상태 업데이트, 영속화 같은
비즈니스 도메인 로직은 Widget 또는 서비스 계층으로 위임해야 합니다.
