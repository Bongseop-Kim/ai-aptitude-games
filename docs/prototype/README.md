...
원본은 standalone HTML 5종(번들 안에 gzip+base64로 소스가 박혀 있음).
...
| `interview-track/` | 실전 면접 트랙 | 실전 영상 면접 트랙 6스텝 플로우: 이력서/채용공고 입력 → AI 분석 → 모의 면접(변형 A/B/C) → 피드백(Pro 잠금) → 재도전 | `iv-app.jsx`, `iv-input.jsx`, `iv-analysis.jsx`, `iv-interview.jsx`, `iv-feedback.jsx`, `iv-shared.jsx` |
