...
function GameStage({ ctx, g, round, total, score, mock, idx, children, footer, instruction, fixedSecs }) {
  const [secs, setSecs] = useS(fixedSecs != null ? fixedSecs : (ctx && ctx.board ? 8 : 0));
  useE(() => { if (fixedSecs != null || (ctx && ctx.board)) return; const t = setInterval(() => setSecs(s => s + 1), 1000); return () => clearInterval(t); }, []);
